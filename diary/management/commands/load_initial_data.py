    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(is_superuser=True).exists():
            ...
            try:
                ...
                if not admin_user.is_superuser:
                    ...
                    admin_user.save()
                    self.stdout.write(self.style.SUCCESS(f'Updated existing user to superuser: {admin_email}'))
                else:
                ...
                self.stdout.write(self.style.SUCCESS(f'Created new superuser: {admin_email}'))
        else:
            admin_user = User.objects.filter(is_superuser=True).first()
            self.stdout.write(self.style.WARNING(f'Using existing superuser: {admin_user.email}'))
        ...
        sample_entries = [
            ...
            },
            {
                ...
                'content': 'Сегодня пришла в голову интересная идея для нового проекта. Нужно записать ее пока не забыл:\n\n- Создать приложение для учета личных финансов\n- Добавить возможность анализа расходов\n- Синхронизация с банковскими картами',
                'mood': 5,
                'tags': ['Идеи', 'Финансы']
            },
            {
                'title': 'Прочитал интересную книгу',
                'content': 'Закончил читать "Атомные привычки". Очень понравилась идея о том, что небольшие ежедневные изменения могут привести к значительным результатам в долгосрочной перспективе.',
                'mood': 4,
                'tags': ['Книги', 'Личное']
            },
            {
                'title': 'Планы на отпуск',
                'content': 'Начинаю планировать отпуск. Хочу посетить:\n\n- Италию\n- Грецию\n- Испанию\n\nНужно изучить варианты перелета и проживания.',
                'mood': 5,
                'tags': ['Путешествия']
            },
            {
                'title': 'Посетил спортзал',
                'content': 'После долгого перерыва снова посетил спортзал. Чувствую себя уставшим, но довольным. Нужно сделать это привычкой!',
                'mood': 4,
                'tags': ['Здоровье']
            },
            {
                'title': 'Изучаю Django',
                'content': 'Потратил несколько часов на изучение Django. Очень мощный фреймворк! Особенно понравилась система ORM и административная панель.',
                'mood': 4,
                'tags': ['Учеба', 'Работа']
            },
            {
                'title': 'Финансовый отчет за месяц',
                'content': 'Проанализировал расходы за прошлый месяц. Нужно сократить траты на развлечения и больше откладывать на будущие цели.',
                'mood': 3,
                'tags': ['Финансы']
            }
        ]
        ...
        for i, entry_data in enumerate(sample_entries):
            ...
            self.stdout.write(self.style.SUCCESS(f'Created entry: {entry_data["title"]}'))

        self.stdout.write(self.style.SUCCESS('Successfully loaded initial data with sample entries'))